                        ...
                        if dns.qr == 1:
                            for rr in dns.an:
                                if rr.type == 1:
                                    ...
                                    dns = dns.qd[0].name
                                    cnx = mysql.connector.connect(user='root', password='desaw2', host='127.0.0.1',
                                                                  database='ipfit5')
                                    cursor = cnx.cursor()
                                    query = (
                                    "INSERT INTO records (id,ipadres, dns, timestamp, ttl, geolocation, idProject) VALUES (%s, %s, %s, %s, %s, %s, %s)")
